...
function Checkout() {
  ...
  const validateForm = () => {
    ...
    const phoneRegex =
      /^(0|\+84)(3[2-9]|5[6|8|9]|7[06-9]|8[1-9]|9[0-9])[0-9]{7}$/;
    ...
    try {
      ...
      if (result.valid) {
        ...
        if (couponData.min_purchase && totalAmount < couponData.min_purchase) {
          setCoupon(null);
          setCouponError(
            `Đơn hàng tối thiểu phải đạt ${formatVND(
  ...
  const calculatePrice = (item: ICartItem) => {
    //  Tính giá sản phẩm
    const variant =
      item.variant && typeof item.variant !== "string" ? item.variant : null;
  ...
  const finalTotal = Math.max(
    0,
    totalAmount + deliveryCharges - discountAmount
  );
    ...
    try {
      const orderData = {
        ...
        items: selectedItems.map((item) => {
          ...
          const variant =
            typeof item.variant === "string" ? null : item.variant;
          const { price } = calculatePrice(item);
        ...
        finalAmount: Math.max(0, finalTotal), // ✅ đảm bảo không âm
    ...
    <div className="bg-gray-100 min-h-screen py-10">
      ...
      <div className="flex justify-center mt-8">
        <p className="flex items-center gap-2 text-sm text-gray-700 bg-blue-50 border border-blue-200 rounded-lg px-6 py-3 shadow max-w-3xl w-full justify-center text-center">
          ...
          <span>
            Thông tin đơn hàng sẽ được gửi qua email. Nhà cung cấp sẽ liên hệ
            với bạn qua số điện thoại.
          </span>
        </p>
      </div>
    </div>
